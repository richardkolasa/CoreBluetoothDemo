...
- (void)viewDidLoad
{
	...
	NSArray *services = @[[CBUUID UUIDWithString:PULSE_OX_SERVICE_UUID], [CBUUID UUIDWithString:POLARH7_HRM_DEVICE_INFO_SERVICE_UUID]];
...
{
	if ([service.UUID isEqual:[CBUUID UUIDWithString:POLARH7_HRM_PULSE_OX_CHARACTERISTIC_UUID]])  {  // 1
...
//			}
	...
	if ([service.UUID isEqual:[CBUUID UUIDWithString:PULSE_OX_SERVICE_UUID]])  { // 5
        for (CBCharacteristic *aChar in service.characteristics)
        {
            if ([aChar.UUID isEqual:[CBUUID UUIDWithString:POLARH7_HRM_PULSE_OX_CHARACTERISTIC_UUID]]) {
                [self.polarH7HRMPeripheral readValueForCharacteristic:aChar];
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
	...
	if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:POLARH7_HRM_PULSE_OX_CHARACTERISTIC_UUID]]) { // 1
